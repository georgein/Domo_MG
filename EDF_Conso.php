...
//Variables :
	...
	$fileExportJS = (getRootPath() . "$pathRef/js/Conso_EDF.js"); 
...
//	mg::message('', "debug ============> cpt_Mois : $cpt_Mois - Mois - $mois / $annee <=============================");
	$mois++;
	if ($mois > 12) {
		$mois = 1;
		$annee = date("Y");
...
$annee = date("Y");
$mois = date('m');
//mg::message('', "debug ============> cpt_Mois : $cpt_Mois courant : Mois - $mois / $annee <=============================");
...
function ExtraitConsoMois($mois, $annee, $detail_Lignes, $cpt_Mois, &$numLigne, $autres = false) {
	...
	foreach ($tabConso as $equipement => $detailsConso) {
		...
		// Calcul la conso du mois de l'équipement etat
			...
			// Si ENEDIS Correction du décalage horaire de la mesure via $valueDate
			if ($typeResult == 'E') {
//				$consoMois = $consoMois + ($consoMois / date('d') / 24 * date('G')); 
				$tmp = mg::getCmd($infCmdConsommation, '', $collectDate, $valueDate);
				$consoMois = $consoMois + ($consoMois / (date('d', $valueDate) /24) * (date('d') * 24 + date('G'));
mg::message('', "************ $tmp - $consoMois *******************");				
return;
			}
